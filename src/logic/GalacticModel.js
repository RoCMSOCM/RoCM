...
GalacticModel.prototype = {
	GR: function(R) {
		...
		var rotation_velocity = Math.sqrt(R * (((Nstar*B*c*c*R)/(2*R0km*R0km*R0km)) * bessel_func));
	...
	CONFORMAL: function(R) {
		...
		var R0kpc = PARAMS.get("R0"); 
		var R0km = CONVERT.kpc_to_km(R0kpc);

		var Nstar = PARAMS.get("N*");
		var Ng = PARAMS.get("Ng");
	...
	MOND: function(R) {
		...
		var M = PARAMS.get("N*") * CONST.get("Mo");	//TODO: Add M_HI	
